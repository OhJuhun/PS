...
void make_maxTree(int cidx) {
	int nidx = cidx / 2;
	while (nidx >= 1) {
		maxtree[nidx] = max(maxtree[nidx * 2], maxtree[nidx * 2 + 1]);
		nidx++;
		if (nidx == cidx) {
			nidx /= 4;
			cidx /= 2;
...
int findMax(int r, int m, int n, int s, int e) {
	if (s <= m && n <= e) return maxtree[r];

	if (e < m || n < s) return 0;
	return max(findMax(2 * r, m, (n + m) / 2, s, e), findMax(2 * r + 1, (n + m) / 2 + 1, n, s, e));
}
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		int k;
		cin >> k;
		maxtree[startidx + i] = k;
		mintree[startidx + i] = k;
	}
	make_minTree(startidx);
	make_maxTree(startidx);
	for (int i = 0; i < m; i++) {
		int s, e;
		cin >> s >> e;
		cout << findMin(1, 1, startidx, s, e) << " "<<findMax(1, 1, startidx, s, e)<<'\n';
	}
	return 0;
}